...
def api(method, url, body=None, token_str=None, retries=3):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            logging.warning('Silent exception caught in release.release: urllib.error.HTTPError')
            return (exc.code, exc.read())
        except Exception as exc:
            logging.warning('Silent exception caught in release.release: Exception')
            last = exc
            if attempt < retries:
...
def asset_names(tag):
    ...
    return ['ReadMDSetup-v%s.exe' % version, 'ReadMD-portable-v%s.exe' % version, 'ReadMD-macos-x64-v%s.zip' % version, 'ReadMD-macos-arm64-v%s.zip' % version, 'SHA256SUMS.txt']
...
def verify_release(args, tok):
    ...
    for name in asset_names(args.tag):
        ...
        print('%-7s %s  size=%s  sha256=%s' % (status, name, asset.get('size', 0), 'MATCH' if path and local_ok else 'REMOTE' if digest_ok else 'MISSING'))
