...
rem  and ReadMDUninstall.exe (uninstaller). Requires dist\ReadMD.exe.
rem ============================================================
setlocal
cd /d "%~dp0.."
title ReadMD Setup Builder

echo [1/4] Checking venv and dist\ReadMD.exe ...
if not exist ".venv\Scripts\python.exe" (
    python -m venv .venv
    ".venv\Scripts\python.exe" -m pip install --disable-pip-version-check -q -r requirements.txt pyinstaller
)
if not exist "dist\ReadMD.exe" (
    echo ReadMD.exe not found. Run package.bat first.
    pause
    exit /b 1
)
...
echo [4/4] Building ReadMDSetup.exe ...
".venv\Scripts\python.exe" -m PyInstaller --noconfirm --clean --onefile --windowed ^
    --name ReadMDSetup --icon "assets\readmd.ico" ^
    --add-data "installer;installer" ^
    --add-binary "dist\ReadMD.exe;." ^
