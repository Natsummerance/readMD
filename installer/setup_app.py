...
try:
    ...
    logging.warning('Silent exception caught in installer.setup_app: ImportError')
...
try:
    ...
    logging.warning('Silent exception caught in installer.setup_app: Exception')
APP_NAME = 'ReadMD'
APP_EXE = 'ReadMD.exe'
...
def _bundle_version():
    ...
    try:
        ...
        logging.warning('Silent exception caught in installer.setup_app: Exception')
    return None
APP_VERSION = os.environ.get('READMD_VERSION_OVERRIDE') or os.environ.get('READMD_BUILD_VERSION') or _bundle_version() or '2.3.3'
PUBLISHER = 'Natsummerance'
PROG_ID = 'ReadMD.markdown'
EXTENSIONS = ['.md', '.markdown', '.mdown', '.mkd']
RELEASE_URL = 'https://github.com/Natsummerance/readMD/releases'
INSTALL_STEPS = [('prepare', '准备安装目录'), ('copy', '复制程序文件'), ('runtime', '安装 WebView2 运行时'), ('assoc', '注册文件关联'), ('shortcut', '创建快捷方式'), ('uninst', '写入卸载信息')]
UNINSTALL_STEPS = [('stop', '关闭运行中的 ReadMD'), ('assoc', '移除文件关联'), ('shortcut', '删除快捷方式'), ('uninst', '移除卸载信息'), ('delete', '清理安装目录')]
UNINSTALL_KEY = 'Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\ReadMD'
ELEVATION_SCHEMA = 1
...
class InstallError(RuntimeError):
    ...
    def as_dict(self):
        return {'ok': False, 'code': self.code, 'message': str(self), 'path': self.path, 'actions': self.actions}

def _result(ok=True, code='ok', path='', message='', actions=None, **extra):
    result = {'ok': ok, 'code': code, 'path': path, 'message': message, 'actions': list(actions or [])}
    result.update(extra)
    return result
...
WEBVIEW2_CLIENT_GUID = 'Software\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}'
...
def system_webview2_installed():
    ...
    for root in (winreg.HKEY_CURRENT_USER, winreg.HKEY_LOCAL_MACHINE):
        try:
            ...
            logging.warning('Silent exception caught in installer.setup_app: OSError')
    return False
...
def bundled_app_dir():
    ...
    for cand in (os.path.join(sys._MEIPASS, 'ReadMD') if getattr(sys, '_MEIPASS', None) else None, os.path.join(asset_root(), 'dist', 'ReadMD')):
...
def app_running():
    try:
        r = subprocess.run(['tasklist', '/FI', 'IMAGENAME eq %s' % APP_EXE], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
...
def stop_app():
    try:
        subprocess.run(['taskkill', '/IM', APP_EXE, '/F'], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
        time.sleep(0.6)
    except Exception:
        logging.warning('Silent exception caught in installer.setup_app: Exception')
...
def _is_child(path, parent):
    try:
        ...
        logging.warning('Silent exception caught in installer.setup_app: ValueError')
        return False
...
def _protected_install_path(path):
    ...
    if local and _is_child(path, local):
        return False
    protected = [os.environ.get('ProgramFiles'), os.environ.get('ProgramFiles(x86)'), os.environ.get('WINDIR'), os.environ.get('SystemRoot')]
...
def _bundle_size():
    ...
    if root:
        try:
            return sum((os.path.getsize(os.path.join(base, f)) for (base, _dirs, files) in os.walk(root) for f in files))
        except OSError:
            logging.warning('Silent exception caught in installer.setup_app: OSError')
...
def preflight_install(directory, options=None, *, disk_usage=shutil.disk_usage, running_check=app_running, admin_check=_is_admin, write_probe=True):
    ...
    try:
        ...
        logging.warning('Silent exception caught in installer.setup_app: InstallError')
        return exc.as_dict()
    ...
    if _protected_install_path(path) and (not admin_check()):
        return _result(False, 'requires_admin', path, '此目录受 Windows 保护，需要管理员权限。', ['elevate', 'change_dir'])
    try:
        os.makedirs(parent, exist_ok=True)
    except PermissionError:
        logging.warning('Silent exception caught in installer.setup_app: PermissionError')
        return _result(False, 'permission_denied', parent, '无法创建或写入安装目录。', ['elevate', 'change_dir'])
    ...
    try:
        ...
        if free < required:
            return _result(False, 'no_space', path, '可用空间不足。', ['change_dir'], free_bytes=free, required_bytes=required)
    except OSError:
        logging.warning('Silent exception caught in installer.setup_app: OSError')
    if write_probe:
        probe = None
        try:
            ...
            logging.warning('Silent exception caught in installer.setup_app: OSError')
            return _result(False, 'permission_denied', parent, '目录写入测试失败：%s' % exc, ['elevate', 'change_dir'])
        finally:
    ...
    if running and (not options.get('force')):
        return _result(False, 'file_in_use', path, 'ReadMD 正在运行，请关闭后再升级。', ['close_app_retry', 'change_dir'], old_version=old, running=True)
...
def create_elevation_payload(options, *, temp_dir=None, now=None, token=None):
    ...
    payload = {'schema': ELEVATION_SCHEMA, 'token': token, 'expires_at': now + ELEVATION_TTL_SECONDS, 'target_dir': target, 'options': dict(options or {}, dir=target)}
...
def _payload_owner_is_current_user(path):
    ...
    if os.name != 'nt':
        try:
            ...
            logging.warning('Silent exception caught in installer.setup_app: (AttributeError, OSError)')
            return False
    try:
        if os.path.islink(path):
            return False
        script = '$acl = Get-Acl -LiteralPath $args[0]; $me = [Security.Principal.WindowsIdentity]::GetCurrent().Name; Write-Output $acl.Owner; Write-Output $me'
        result = subprocess.run(['powershell.exe', '-NoProfile', '-NonInteractive', '-Command', script, path], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW, timeout=5)
        ...
        logging.warning('Silent exception caught in installer.setup_app: (OSError, subprocess.SubprocessError)')
        return False
...
def consume_elevation_payload(path, token, *, temp_dir=None, now=None, owner_check=_payload_owner_is_current_user):
    ...
    try:
        ...
        logging.warning('Silent exception caught in installer.setup_app: (OSError, ValueError)')
        ...
        try:
            ...
            logging.warning('Silent exception caught in installer.setup_app: OSError')
    now = time.time() if now is None else now
    if not isinstance(payload, dict) or payload.get('schema') != ELEVATION_SCHEMA or (not secrets.compare_digest(str(payload.get('token', '')), str(token or ''))) or (not isinstance(payload.get('expires_at'), (int, float))) or (payload['expires_at'] < now):
...
def backup_assoc():
    ...
    if reg_get('Software\\Classes\\.md') is not None:
        subprocess.run(['reg', 'export', 'HKCU\\Software\\Classes\\.md', bak, '/y'], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
        return bak
    return None
...
def write_assoc(inst_dir):
    ...
    for ext in EXTENSIONS:
        reg_set('Software\\Classes\\%s' % ext, '', PROG_ID)
    reg_set('Software\\Classes\\%s' % PROG_ID, '', 'ReadMD Markdown Reader')
    reg_set('Software\\Classes\\%s\\DefaultIcon' % PROG_ID, '', '"%s",0' % icon)
    reg_set('Software\\Classes\\%s\\shell\\open\\command' % PROG_ID, '', '"%s" "%%1"' % exe, typ=winreg.REG_EXPAND_SZ)
    reg_set('Software\\Classes\\%s\\shell\\openwith' % PROG_ID, '', '')
    reg_set('Software\\Classes\\Applications\\%s\\shell\\open\\command' % APP_EXE, '', '"%s" "%%1"' % exe, typ=winreg.REG_EXPAND_SZ)
    try:
        subprocess.run(['ie4uinit.exe', '-show'], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
    except Exception:
        logging.warning('Silent exception caught in installer.setup_app: Exception')
...
def remove_assoc():
    ...
    if os.path.isfile(bak):
        subprocess.run(['reg', 'import', bak], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
    for ext in EXTENSIONS:
        if reg_get('Software\\Classes\\%s' % ext) == PROG_ID:
            reg_del_tree('Software\\Classes\\%s' % ext)
    reg_del_tree('Software\\Classes\\%s' % PROG_ID)
    reg_del_tree('Software\\Classes\\Applications\\%s' % APP_EXE)

def get_special_folder(name):
    try:
        r = subprocess.run(['powershell.exe', '-NoProfile', '-NonInteractive', '-Command', '[Environment]::GetFolderPath("%s")' % name], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
...
def create_shortcut(target, lnk_path, icon):
    try:
        ps = '$ws = New-Object -ComObject WScript.Shell;$s = $ws.CreateShortcut("%s");$s.TargetPath = "%s";$s.IconLocation = "%s,0";$s.Description = "ReadMD Markdown Reader";$s.Save()' % (lnk_path, target, icon)
        subprocess.run(['powershell.exe', '-NoProfile', '-NonInteractive', '-Command', ps], capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
    except Exception:
        logging.warning('Silent exception caught in installer.setup_app: Exception')
...
def _copy_file(src, dst, optional=False):
    ...
    for i in range(5):
        try:
            ...
            logging.warning('Silent exception caught in installer.setup_app: PermissionError')
            time.sleep(0.5)
    raise InstallError('file_in_use', '文件被占用，无法写入：%s' % dst, dst, ['close_app_retry', 'change_dir'])
...
def _write_install_manifest(inst_dir, opts):
    with open(os.path.join(inst_dir, 'install.json'), 'w', encoding='utf-8') as f:
        json.dump({'app': APP_NAME, 'version': APP_VERSION, 'installed_at': time.strftime('%Y-%m-%d %H:%M:%S'), 'dir': inst_dir, 'assoc': bool(opts.get('assoc', True)), 'webview2': bool(opts.get('webview2', False))}, f, ensure_ascii=False, indent=2)
...
def _commit_staged_install(stage_dir, inst_dir):
    ...
    try:
        ...
        try:
            ...
            logging.warning('Silent exception caught in installer.setup_app: Exception')
            raise InstallError('rollback_failed', '替换安装失败，且旧版本恢复失败：%s' % rollback_error, inst_dir, ['change_dir']) from exc
        code = 'file_in_use' if isinstance(exc, PermissionError) else 'replace_failed'
        raise InstallError(code, '无法替换当前安装，旧版本已保留：%s' % exc, inst_dir, ['close_app_retry', 'elevate', 'change_dir']) from exc
...
def do_install(opts, progress):
    ...
    if opts.get('force') and app_running():
        stop_app()
        if app_running():
            raise InstallError('file_in_use', 'ReadMD 仍在运行，无法升级。', inst_dir, ['close_app_retry', 'change_dir'])
    ...
    try:
        ...
        logging.warning('Silent exception caught in installer.setup_app: PermissionError')
        raise InstallError('permission_denied', '安装目录没有写入权限。', getattr(exc, 'filename', inst_dir), ['elevate', 'change_dir']) from exc
    except OSError as exc:
        logging.warning('Silent exception caught in installer.setup_app: OSError')
        raise InstallError('install_failed', '安装失败：%s' % exc, getattr(exc, 'filename', inst_dir), ['close_app_retry', 'change_dir']) from exc
    finally:
...
class Api(object):
    ...
    def choose_dir(self, current=''):
        try:
            import webview
            return self._window.create_file_dialog(webview.FOLDER_DIALOG, current or default_install_dir())
        except Exception:
            logging.warning('Silent exception caught in installer.setup_app: Exception')
            return None
    ...
    def start_install(self, opts):
        ...
        if not check['ok']:
            with self._lock:
                self._state['progress'] = {'running': False, 'percent': 0, 'step': 'error', 'text': '安装前检查未通过', 'done': False, 'error': check['message'], 'code': check['code'], 'path': check['path'], 'actions': check['actions']}
            return check
    ...
    def launch(self):
        ...
        if inst:
            try:
                subprocess.Popen([os.path.join(inst, APP_EXE)], creationflags=subprocess.CREATE_NO_WINDOW)
            except Exception:
                logging.warning('Silent exception caught in installer.setup_app: Exception')
                return False
            return True
    ...
    def _run_install(self, opts):
        ...
        def cb(p, step, text):
            with self._lock:
                self._state['progress'] = {'running': True, 'percent': p, 'step': step, 'text': text, 'done': False, 'error': '', 'code': '', 'path': '', 'actions': []}
        try:
            do_install(opts, cb)
            with self._lock:
                self._state['progress'] = {'running': False, 'percent': 100, 'step': 'done', 'text': '安装完成', 'done': True, 'error': '', 'code': '', 'path': '', 'actions': []}
                self._state['installed'] = is_installed()
        except InstallError as e:
            logging.warning('Silent exception caught in installer.setup_app: InstallError')
            with self._lock:
                self._state['progress'] = {'running': False, 'percent': 0, 'step': 'error', 'text': '安装失败', 'done': False, 'error': str(e), 'code': e.code, 'path': e.path, 'actions': e.actions}
        except Exception as e:
            logging.warning('Silent exception caught in installer.setup_app: Exception')
            with self._lock:
                self._state['progress'] = {'running': False, 'percent': 0, 'step': 'error', 'text': '安装失败', 'done': False, 'error': str(e), 'code': 'install_failed', 'path': '', 'actions': ['close_app_retry', 'change_dir']}
    ...
    def _run_uninstall(self):
        ...
        try:
            do_uninstall(cb)
            with self._lock:
                self._state['progress'] = {'running': False, 'percent': 100, 'step': 'done', 'text': '卸载完成', 'done': True, 'error': '', 'code': '', 'path': '', 'actions': []}
                self._state['installed'] = is_installed()
        except Exception as e:
            logging.warning('Silent exception caught in installer.setup_app: Exception')
            with self._lock:
                self._state['progress'] = {'running': False, 'percent': 0, 'step': 'error', 'text': '卸载失败', 'done': False, 'error': str(e), 'code': 'uninstall_failed', 'path': '', 'actions': ['change_dir']}
...
def run_gui(uninstall_mode):
    import webview
    inst = is_installed()
    state = {'mode': 'uninstall' if uninstall_mode or is_uninstaller() else 'install', 'installed': inst, 'default_dir': (inst or {}).get('dir') or default_install_dir(), 'version': APP_VERSION, 'running': app_running(), 'win7': is_win7(), 'webview2Default': is_win7() and (not system_webview2_installed()), 'progress': {'running': False, 'percent': 0, 'step': '', 'text': '', 'done': False, 'error': '', 'code': '', 'path': '', 'actions': []}}
    if state['mode'] == 'uninstall' and inst is None:
        state['mode'] = 'install'
    api = Api(state)
    url = 'http://127.0.0.1:%d/installer/setup.html' % server_port
    try:
        window = webview.create_window('ReadMD 安装程序', url, js_api=api, width=760, height=520, min_size=(560, 380), resizable=True, text_select=True, background_color='#0a0e18')
    except Exception:
        logging.warning('Silent exception caught in installer.setup_app: Exception')
        window = webview.create_window('ReadMD 安装程序', url, js_api=api, width=760, height=520, min_size=(560, 380), text_select=True, background_color='#0a0e18')
    api._window = window
    rt = bundled_webview2_runtime_dir()
    if rt is not None:
...
def main():
    ...
    if args.install_silent is not None:
        ...
        return run_install_silent({'dir': d, 'assoc': True, 'desktop': False, 'startmenu': False, 'force': args.force, 'webview2': is_win7() and (not system_webview2_installed())})
