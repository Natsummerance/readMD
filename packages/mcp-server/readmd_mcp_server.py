...
"""
# Version metadata for consistency checks: "version": "2.3.3"
import sys
import os
import json
import logging
...
try:
    ...
    logging.warning('Silent exception caught in packages.mcp-server.readmd_mcp_server: ImportError')
TOOLS = [{'name': 'readmd_fix_markdown', 'description': '自动修复 Markdown 文本中的各类语法格式错误（公式断裂、表格错位、代码块未闭合、中英文标点空格等）。', 'inputSchema': {'type': 'object', 'properties': {'content': {'type': 'string', 'description': '待修复的 Markdown 原始内容'}}, 'required': ['content']}}, {'name': 'readmd_convert_to_markdown', 'description': '将本地各种格式的文档（docx, pdf, pptx, xlsx, tex, html, epub 等）转换为高质量 Markdown。', 'inputSchema': {'type': 'object', 'properties': {'file_path': {'type': 'string', 'description': '本地文档的绝对路径'}}, 'required': ['file_path']}}, {'name': 'readmd_latex_to_md', 'description': '将 LaTeX 论文或公式源码转换为干净的标准 Markdown 格式。', 'inputSchema': {'type': 'object', 'properties': {'latex_content': {'type': 'string', 'description': 'LaTeX 源代码'}}, 'required': ['latex_content']}}, {'name': 'readmd_md_to_latex', 'description': '将 Markdown 文档转换/编译为可直接供 pdflatex / xelatex 编译的学术 LaTeX 源码。', 'inputSchema': {'type': 'object', 'properties': {'markdown_content': {'type': 'string', 'description': 'Markdown 文本内容'}, 'doc_title': {'type': 'string', 'description': '文档标题（可选）'}}, 'required': ['markdown_content']}}, {'name': 'readmd_parse_bibtex', 'description': '扫描并解析 BibTeX (.bib) 参考文献文件，提取结构化论文元数据与标准引用。', 'inputSchema': {'type': 'object', 'properties': {'bib_file_path': {'type': 'string', 'description': '.bib 文件路径或包含 .bib 的目录路径'}}, 'required': ['bib_file_path']}}]

def handle_tool_call(name, args):
    if name == 'readmd_fix_markdown':
        content = args.get('content', '')
        res = readmd_fix.fix_markdown(content)
        return {'content': [{'type': 'text', 'text': json.dumps({'ok': True, 'repaired_content': res.text, 'fixes_count': len(res.fixes), 'fixes_details': res.fixes, 'stats': res.stats}, ensure_ascii=False, indent=2)}]}
...
def run_stdio_server():
    ...
    if sys.platform == 'win32' and hasattr(sys.stdin, 'reconfigure'):
        try:
            ...
            logging.warning('Silent exception caught in packages.mcp-server.readmd_mcp_server: Exception')
    while True:
        try:
            ...
            if method == 'initialize':
                res = {'jsonrpc': '2.0', 'id': req_id, 'result': {'protocolVersion': '2024-11-05', 'serverInfo': {'name': 'readmd-mcp-server', 'version': '2.3.3'}, 'capabilities': {'tools': {}}}}
