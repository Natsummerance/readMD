...
def evaluate_startup_reports(reports, max_page_loaded_ms, max_window_overhead_ms):
    ...
    page_median = statistics.median(item['page_loaded_ms'] for item in samples) if samples else None
    window_overhead_median = statistics.median((
        item['server_to_window_ms'] for item in samples
    )) if samples else None
    if page_median is not None and page_median > max_page_loaded_ms:
        failures.append(f'page_loaded median {page_median:.1f}ms exceeds {max_page_loaded_ms}ms')
    if window_overhead_median is not None and window_overhead_median > max_window_overhead_ms:
        failures.append(
            f'server-to-window median {window_overhead_median:.1f}ms exceeds {max_window_overhead_ms}ms'
        )
    ...
    return {
        ...
        'budgets_ms': {
            'page_loaded_median': max_page_loaded_ms,
            'server_to_window_median': max_window_overhead_ms,
        },
        'measured_ms': {
            'page_loaded_median': page_median,
            'server_to_window_median': window_overhead_median,
        },
        'samples': samples,
        'failures': failures,
    }
