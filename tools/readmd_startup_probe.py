...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='readmd-startup-probe-') as directory:
        for index in range(args.runs):
            ...
            result = subprocess.run([sys.executable, app, '--startup-probe', '--startup-probe-json', output, '--startup-probe-timeout', str(args.timeout)], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
            report = {'run': index + 1, 'returncode': result.returncode}
            try:
                with open(output, encoding='utf-8') as handle:
                    report.update(json.load(handle))
            except OSError:
                logging.warning('Silent exception caught in tools.readmd_startup_probe: OSError')
