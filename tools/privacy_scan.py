...
KEY_PATTERNS = [re.compile(b'\\bsk-[A-Za-z0-9_-]{20,}\\b'), re.compile(b'\\bAIza[0-9A-Za-z_-]{30,}\\b')]
LOCAL_PATH_PATTERNS = [re.compile(b'[a-zA-Z]:[/\\\\](?:users|programming|project|workspace|home|desktop|downloads)', re.IGNORECASE)]
...
def scan_file(path, label, failures):
    try:
        ...
        logging.warning('Silent exception caught in tools.privacy_scan: OSError')
        return
    lower = data.lower()
