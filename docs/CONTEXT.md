...
> 当前工作基线为 `2.3.8-preview.1` 隔离候选；下文历史版本号仅用于追溯，不能作为当前发布状态或平台验收证据。
...
- 当前开发/发布目标：v2.3.8 Preview（先完成当前候选验收；平台原生证据、签名/公证和启动性能门禁通过后才可正式发布）
...
8. 一键部署：deploy.bat + release.py（--verify/--update），CI 创建 Release；本地工具只维护既有 Release
