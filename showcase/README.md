...
正式包会把选中的 `poster_style` 写进 `story.json`、`metadata.json` 和 schema 3 的 `composition.json`；watcher 再把它作为不可变身份写入发布账本。绩效报告会按风格聚合曝光和加权互动，置信不足时只报告探索结果，不会自动替换默认的 evidence-paper。
