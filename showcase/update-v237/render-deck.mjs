...
function parseArgs(argv) {
  const result = { locale: 'zh-CN', input: path.join(showcaseRoot, 'raw'), output: path.join(showcaseRoot, 'output', 'v237-update'), verifyOnly: false };
...
function posterHtml(page, index, total, locale, source) {
  ...
  const footerScope = isChinese ? 'V2.3.6 → V2.3.7 · 本地优先的文档工作流' : 'V2.3.6 → V2.3.7 · LOCAL-FIRST DOCUMENT WORK';
...
<style>
  ...
  *{box-sizing:border-box} html,body{width:1080px;height:1440px;margin:0;overflow:hidden;background:var(--paper);color:var(--ink)}
  ...
  /* Evidence-first overrides: a widescreen, untrimmed product capture must own at
     least 30% of a vertical poster. The annotations stay editorial, but never
     force an actual UI screenshot into a decorative thumbnail. */
...
<article class="poster ${page.layout}">
  ...
  <section class="page-body">
    ...
    <figure class="evidence"><figcaption class="evidence-label">READMD 2.3.7 <span>${evidenceStatus}</span></figcaption><div class="frame"><img src="${source}" alt="${title}"></div></figure>
  </section>
...
async function main() {
  ...
  if (options.verifyOnly) {
    console.log(`Verified ${pages.length} distinct authentic V2.3.7 screenshot sources.`);
    return;
  }
