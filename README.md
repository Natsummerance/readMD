...
- 🤖 **AI 助手**：接入外部大模型 API（OpenAI / DeepSeek / Kimi / 智谱 GLM / 通义千问 / 硅基流动 / OpenRouter / Groq / xAI / Ollama / Anthropic 等 15+ 预设，并兼容 cc-switch 里的自定义提供商），支持快速阅读、修改、扩充、续写、润色、翻译、提问；流式输出，结果可一键应用到文档；提供商与 API Key 完全自定义（Key 也可读系统环境变量）
