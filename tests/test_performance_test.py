...
class RegistryTest(unittest.TestCase):
    ...
    def test_whitelist_and_concurrent_idempotence(self):
        ...
        def fake_import(name, package=None):
            ...
            full_name = name if not package else (package.rstrip('.') + '.' + name.lstrip('.'))
            calls.append(full_name)
            entered.set()
            release.wait(1)
            return types.SimpleNamespace(load=lambda : None)
        with mock.patch('importlib.import_module', fake_import), mock.patch.object(self.registry.logging, 'exception'):
            threads = [threading.Thread(target=lambda : self.registry.load('convert')) for _ in range(12)]
            for thread in threads:
                thread.start()
            self.assertTrue(entered.wait(1))
            self.assertIn('src.readmd_modules.convert', calls)
            release.set()
            for thread in threads:
                thread.join(1)
...
class HttpModuleTest(unittest.TestCase):
    ...
    def request(self, path, body=None):
        req = urllib.request.Request(self.base + path, data=body, method='POST' if body is not None else 'GET', headers={'Content-Type': 'application/json'} if body else {})
        try:
            with urllib.request.urlopen(req, timeout=3) as response:
                return (response.status, json.loads(response.read().decode('utf-8')))
        except urllib.error.HTTPError as exc:
            logging.warning('Silent exception caught in tests.test_performance_test: urllib.error.HTTPError')
            return (exc.code, json.loads(exc.read().decode('utf-8')))

    def test_get_status_does_not_load_and_post_loads_one_module(self):
        calls = []
        with mock.patch.object(readmd.RM, 'load', side_effect=lambda name: calls.append(name) or 'loading'), mock.patch.object(readmd.RM, 'status', return_value=({'convert': 'idle', 'ocr': 'idle', 'web': 'idle', 'ai': 'idle'}, {})):
    ...
    def test_tray_is_deferred_and_created_once(self):
        ...
        try:
            ...
            with mock.patch.object(readmd, '_start_tray', side_effect=lambda window: calls.append(window) or 'tray'):
                threads = [threading.Thread(target=lambda : readmd._start_tray_once('window')) for _ in range(8)]
                for thread in threads:
                    thread.start()
                for thread in threads:
                    thread.join(1)
